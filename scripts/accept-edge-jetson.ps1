...
function Add-AcceptanceCheck {
    param([string]$Name, [ValidateSet("passed", "failed", "pending", "skipped")][string]$Status, [string]$Detail = "", [object]$Evidence = $null)
    $checks.Add([ordered]@{ name = $Name; status = $Status; detail = $Detail; evidence = $Evidence }) | Out-Null
}

function Invoke-ApiJson {
    param([ValidateSet("GET", "POST", "DELETE")][string]$Method, [string]$Path, [object]$Body = $null)
    $parameters = @{
        Method = $Method
...
function Wait-ApiResource {
    param([string]$Path, [string[]]$SuccessStates, [string[]]$FailureStates)
    ...
    while ([DateTimeOffset]::UtcNow -lt $deadline) {
        ...
        if ($FailureStates -contains $state) { throw "Resource entered failure state '$state': $($resource.error_message)" }
...
function Invoke-ImageInference {
    ...
    try {
        ...
        if (-not $response.IsSuccessStatusCode) { throw "Image inference returned HTTP $([int]$response.StatusCode)" }
...
function Assert-HardwareArguments {
    ...
    foreach ($entry in $required.GetEnumerator()) {
        if ([string]::IsNullOrWhiteSpace([string]$entry.Value)) { throw "-$($entry.Key) is required with -RunHardwareChecks" }
    }
    foreach ($path in @($ServiceRequestJsonPath, $TrainingRequestJsonPath, $InferenceImagePath)) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Required acceptance input does not exist: $path" }
    }
}
...
if (-not $RunHardwareChecks) {
    ...
    try {
        ...
        $scan = Invoke-ApiJson -Method POST -Path "/edge-nodes/scan-host-key" -Body @{ host = $HostName; port = $SshPort }
        ...
        if (-not [bool]$probe.supported -or [string]$probe.inventory.platform_kind -ne "jetson") { throw "Edge inventory is not a supported Jetson host" }
        if ([string]::IsNullOrWhiteSpace([string]$probe.inventory.jetpack_version)) { throw "JetPack version was not detected" }
        if ([string]::IsNullOrWhiteSpace([string]$probe.inventory.l4t_version)) { throw "L4T version was not detected from /etc/nv_tegra_release" }
        Add-AcceptanceCheck -Name "jetpack_l4t" -Status "passed" -Evidence @{
            jetpack_version = $probe.inventory.jetpack_version
            l4t_version = $probe.inventory.l4t_version
        }
        if (-not [bool]$probe.inventory.docker.nvidia_runtime_available) { throw "Docker NVIDIA runtime is unavailable" }
        ...
        if ([string]$service.engine -ne "engine" -or [string]::IsNullOrWhiteSpace([string]$service.engine_digest)) { throw "TensorRT FP16 engine evidence is missing" }
        ...
        else { Add-AcceptanceCheck -Name "cleanup" -Status "skipped" -Detail "Resources retained by -KeepResources." }
    }
    catch { Add-AcceptanceCheck -Name "acceptance_execution" -Status "failed" -Detail $_.Exception.Message }
    finally {
        if ($null -ne $serviceId -and -not $KeepResources) {
            ...
            if (-not $cleanupAlreadyPassed) {
                ...
                catch { Add-AcceptanceCheck -Name "cleanup" -Status "failed" -Detail $_.Exception.Message }
            }
        }
        $PlaintextPassword = $null
